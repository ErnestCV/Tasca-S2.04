...
>Amb la seguent query podem saber quants restaurants de cada tipus de cuina hi ha:
db.restaurants.aggregate({$group: {_id: '$cuisine', count: {$sum: 1}}}, {$sort: {count: -1}});
>Amb la següent, quants tipus de cuina diferents hi ha en total:
db.restaurants.aggregate({$group: {_id: '$cuisine', count: {$sum: 1}}}, {$count: "count"});
*/
db.restaurants.find({$or: [{$and: [{"cuisine": /Seafood/i}, {cuisine: {$nin: ["American ", "Chinese"]}}]}, {"name": /^Wil/}]}, {restaurant_id: 1, name : 1, borough: 1, cuisine: 1, _id: 0});
...
>Podem mirar només si el document no conté el camp address.street:
db.restaurants.find({"address.street": {$exists: false}});
>"address.street": null mira si el document no conté aquest camp o si el camp és buit.
*/
